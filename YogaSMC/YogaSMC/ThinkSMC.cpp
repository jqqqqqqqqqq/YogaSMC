...
void ThinkSMC::addVSMCKey() {
    ...
    VirtualSMCAPI::addKey(KeyF0Ac(0), vsmcPlugin.data, VirtualSMCAPI::valueWithFp(0, SmcKeyTypeFloat, new atomicFpKey(&currentSensor[0])));
    ++sensorCount;
    if (dualFan) {
        VirtualSMCAPI::addKey(KeyF0Dc(0), vsmcPlugin.data, VirtualSMCAPI::valueWithFp(0, SmcKeyTypeFloat, new atomicFpKey(&currentSensor[1])));
        ++sensorCount;
    }
